...
# Optional: curatam de punctuatie
...
print("Unique words", len(all_unique_words))

count_list = [0 for word in all_unique_words]
...
sorted_count = sorted(count_list, reverse=True)

lista_de_numere_de_cuvinte = []
ultimul_indece_pentru_numar = []

print('Most used:\n')

for number in sorted_count[:30]:
    if number in lista_de_numere_de_cuvinte:
        start_from = ultimul_indece_pentru_numar[lista_de_numere_de_cuvinte.index(number)]
        index_of_word = count_list.index(number, start_from + 1)
        ultimul_indece_pentru_numar[lista_de_numere_de_cuvinte.index(number)] = index_of_word
    else:
        index_of_word = count_list.index(number)
        lista_de_numere_de_cuvinte.append(number)
        ultimul_indece_pentru_numar.append(index_of_word)
    print(f'Word {all_unique_words[index_of_word]} was found {number} times')
...
cuvinte = [cuvant.strip(".,:;!?'\"()[]{}") for cuvant in all_words]
# Număr total de cuvinte
total_cuvinte = len(cuvinte)
print("Număr total de cuvinte:", total_cuvinte)

set_cuvinte = set(cuvinte)
result = []
for c in set_cuvinte:
    result.append((c, cuvinte.count(c)))
    sorted_result = sorted(result, key=lambda a: a[1], reverse=True)
top_30 = sorted_result[:30]
last_30 = sorted_result[-30:]
print("Top 30:")
print(top_30)
print("Last 30:")
print(last_30)
